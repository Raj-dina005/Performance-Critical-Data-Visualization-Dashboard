...
export default function ControlPanel({ running, onStart, onStop, onSetRate, onSetBatch }: Props) {
  ...
  // detect worker helper on window (set by OffscreenCanvasView)
  useEffect(() => {
    function check() {
      setWorkerAvailable(typeof (window as any).offscreenWorkerSend === 'function');
    }
    check();
    const t = setInterval(check, 1000); // poll once per second (keeps indicator live)
  ...
  useEffect(() => {
    ...
    if (!stressActive) {
      ...
      setLocalRate(100);
      setLocalBatch(20);
      // inform both local hook and worker (if present)
      onSetRate(100);
      onSetBatch(20);
      (window as any).offscreenWorkerSend?.({ type: 'setOptions', rateMs: 100, batchSize: 20 });
    }
  ...
  function sendToWorker(msg: any) {
    try {
      (window as any).offscreenWorkerSend?.(msg);
    } catch {
  ...
  function runStressToggle() {
    if (!stressActive) {
      ...
      sendToWorker({ type: 'setOptions', rateMs: STRESS_PRESET.rateMs, batchSize: STRESS_PRESET.batch });
      ...
      onSetRate(100);
      onSetBatch(20);
      sendToWorker({ type: 'setOptions', rateMs: 100, batchSize: 20 });
    }
  }
  ...
  function handleRateChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setLocalRate(v);
    onSetRate(v);
    sendToWorker({ type: 'setOptions', rateMs: v });
  ...
  function handleBatchChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setLocalBatch(v);
    onSetBatch(v);
    sendToWorker({ type: 'setOptions', batchSize: v });
    if (stressActive) setStressActive(false);
  }

  function handleClear() {
    // clear both main buffer and worker buffer (if present)
    sendToWorker({ type: 'clear' });
    // no direct hook clear function available here; caller could implement if needed
    // For now, we signal worker and let data stream refill
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 bg-zinc-800 text-white p-3 rounded-lg">
      ...
      <button
        ...
        title="Clear worker buffer"
      >
        Clear
      </button>

      {/* Worker connection indicator + Quick status line */}
      <div className="ml-2 text-xs text-zinc-300">
        <div>
          <span className="mr-2">Worker:</span>
          <span className={`px-2 py-0.5 rounded font-mono ${workerAvailable ? 'bg-emerald-600' : 'bg-red-600'}`}>
            {workerAvailable ? 'ON' : 'OFF'}
          </span>
        </div>
